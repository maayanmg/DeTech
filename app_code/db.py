...
def add_user(name, email, username, password):
    ...
    if len(db_result) == 0:
        doctor_id = str(uuid4())
        connection.execute("INSERT INTO doctors_details_db (full_name, email, username, password, doctor_id) VALUES (?,?,?,?,?)", (name, email, username, password, doctor_id))
        connection.commit()
...
def add_patient_with_photo(full_name, patient_id, email, gender, birth_date, case_description, visit_date, doctor_id, x_ray_detection):
    try:
        # check if the patient is already exists
        cur.execute("SELECT rowid FROM patients_details_db WHERE full_name = ?", (full_name,))
        db_result = cur.fetchall()
        if len(db_result) != 0:
            connection.execute("DELETE FROM patients_details_db WHERE  full_name = '" + full_name + "'")
            connection.commit()
        connection.execute("INSERT INTO patients_details_db (full_name, ID, email, gender, birth_date, case_description, visit_date, doctor_id, x-ray detection) VALUES (?,?,?,?,?,?,?,?,?)", (full_name, patient_id, email, gender, birth_date, case_description, visit_date, doctor_id, x_ray_detection))
        connection.commit()
        return True
...
def find_username_and_password(username, password):
    ...
    if len(db_result) == 0:
        ...
        cur.execute('SELECT doctor_id FROM doctors_details_db')
        data = cur.fetchall()
