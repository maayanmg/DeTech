...
certfile=r"D:\Projects\finale\localhost.pem"
cafile = r"D:\Projects\finale\cacert.pem"
...
def handle_add_patient_msg(msg):
    ...
    if len(msg) < 9:
        ...
        x_ray_detection = msg[8]
        return full_name, patient_id, email, gender, birth_date, case_description, visit_date, doctor_id, x_ray_detection
    ...
    while True:
        try:
            ...
            if split_data[0] == 'ADD_PATIENT':
                full_name, patient_id, email, gender, birth_date, case_description, visit_date, doctor_id, x_ray_detection = handle_add_patient_msg(split_data[1:])
                if x_ray_detection is None:
                    bret = db.add_patient(full_name, patient_id, email, gender, birth_date, case_description, visit_date, doctor_id)
                else:
                    bret = db.add_patient_with_photo(full_name, patient_id, email, gender, birth_date, case_description, visit_date, doctor_id, x_ray_detection)
                if bret:
                    res = str(bret)
                else:
                    res = str(bret)
